...
# 1. 이미지 파일 업로드
# 2. csv 파이 업로드streamlit run app.py


# 디렉토리(폴더)명과 파일을 알려주면,
# 해당 디렉토리에 파일을 저장해 주는 함수!!!!!!!!!!
def save_uploaded_file(directory, file) :
    # 1. 위에 directory가 있는지 먼저 확인해서,
    #      없으면 디렉토릴를 새로 생성하고, 그렇지 않으면 새로 생성 할 필요없다.
...
def main () :
    ...
    if choice == 'Image' :
        ...
        file = st.file_uploader('이미지 파일을 업로드하세요', type= ['jpg', 'jpeg', 'png', 'AVIF'])
        ...
        if file is not None :
            ...
            # 파일이 없로드 되었을때만, 파일 저장하는 코드를 작성한다.
